...
public sealed class DefinitionFixer : CodeFixProvider
{
    ...
    private static async Task RegisterPartialTypeFix(CodeFixContext context, Diagnostic diagnostic)
    {
        ...
        TypeDeclarationSyntax? token = root?.FindToken(span.Start).Parent?.AncestorsAndSelf()
        ...
        CancellationToken cancellation)
    {
        ...
        SyntaxToken token = SyntaxFactory.Token(PartialKeyword);
        TypeDeclarationSyntax newDeclaration = declaration.AddModifiers(token);
    ...
    private static async Task RegisterRedundantTagFix(CodeFixContext context, Diagnostic diagnostic)
    {
        SyntaxNode? root = await context.Document.GetSyntaxRootAsync(context.CancellationToken);
        TextSpan span = diagnostic.Location.SourceSpan;
        MemberDeclarationSyntax? token = root?.FindToken(span.Start).Parent?.AncestorsAndSelf()
        ...
        foreach (AttributeListSyntax attributeList in token.AttributeLists)
        {
        ...
        CancellationToken cancellation)
    {
        ...
        else
        {
            ...
            SeparatedSyntaxList<AttributeArgumentSyntax> newArgs = syntax.ArgumentList.Arguments.RemoveAt(0);
            AttributeArgumentListSyntax newArgList = syntax.ArgumentList.WithArguments(newArgs);
        ...
        CancellationToken cancellation)
    {
        ...
        MemberDeclarationSyntax newSyntax = syntax.WithAttributeLists(newLists);
    ...
    private static async Task RegisterDataFieldFix(CodeFixContext context, Diagnostic diagnostic)
    {
        SyntaxNode? root = await context.Document.GetSyntaxRootAsync(context.CancellationToken);
        TextSpan span = diagnostic.Location.SourceSpan;
        FieldDeclarationSyntax? field = root?.FindToken(span.Start).Parent?.AncestorsAndSelf()
    ...
    private static async Task RegisterDataFieldPropertyFix(CodeFixContext context, Diagnostic diagnostic)
    {
        SyntaxNode? root = await context.Document.GetSyntaxRootAsync(context.CancellationToken);
        TextSpan span = diagnostic.Location.SourceSpan;
        PropertyDeclarationSyntax? property = root?.FindToken(span.Start).Parent?.AncestorsAndSelf()
    ...
    {
        var root = (CompilationUnitSyntax?)await document.GetSyntaxRootAsync(cancellation);
        SyntaxToken token = declaration.Modifiers.First(t => t.IsKind(ReadOnlyKeyword));
        FieldDeclarationSyntax newDeclaration = declaration.WithModifiers(declaration.Modifiers.Remove(token));
    ...
    {
        var root = (CompilationUnitSyntax?)await document.GetSyntaxRootAsync(cancellation);
        PropertyDeclarationSyntax newDeclaration = declaration;
        AccessorDeclarationSyntax? privateSet = newDeclaration
            .AccessorList?
            .Accessors
        ...
        AccessorDeclarationSyntax setter = declaration.Modifiers.Any(m => m.IsKind(PrivateKeyword))
