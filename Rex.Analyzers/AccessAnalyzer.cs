...
{
    private const string AccessAttributeType = "Rex.Shared.Analyzers.AccessAttribute";
    private const string RexAutoGeneratedAttributeType = "Rex.Shared.Analyzers.RexAutoGeneratedAttribute";
    private const string PureAttributeType = "System.Diagnostics.Contracts.PureAttribute";

    [SuppressMessage("ReSharper", "RS2008")]
    private static readonly DiagnosticDescriptor s_accessRule = new(
        IdAccess,
        "Invalid access",
    ...
    private void CheckFriendship(OperationAnalysisContext context)
    {
        ...
        INamedTypeSymbol accessedType = member.ContainingType;

        // Get the attributes
        INamedTypeSymbol friendAttribute = context.Compilation.GetTypeByMetadataName(AccessAttributeType);
        INamedTypeSymbol autoGenAttribute = context.Compilation.GetTypeByMetadataName(RexAutoGeneratedAttributeType);
        ...
        if (accessingType.GetAttributes().FirstOrDefault(a =>
                ...
                a.AttributeClass.Equals(autoGenAttribute, SymbolEqualityComparer.Default)) is not null)
        {
            return;
        }

        // Determine which type of access is happening here... Read, write or execute?
        AccessPermissions accessAttempt = DetermineAccess(context, targetAccess, operation);

        // Check whether this is a "self" access, including inheritors.
        bool selfAccess = InheritsFromOrEquals(accessingType, accessedType);
        ...
        bool CheckAttributeFriendship(AttributeData attribute, bool isMemberAttribute)
        {
            ...
            AccessPermissions self = AccessAttribute.SelfDefaultPermissions;
            AccessPermissions friends = AccessAttribute.FriendDefaultPermissions;
            AccessPermissions others = AccessAttribute.OtherDefaultPermissions;
            ...
            AccessPermissions permissionCheck = others;

            // Short label for the relation between accessing and accessed types.
            string accessingRelation = "other-type";
            ...
            if (!selfAccess)
            {
                ...
                ImmutableArray<TypedConstant> types = attribute.ConstructorArguments[0].Values;

                foreach (TypedConstant constant in types)
                {
            ...
            context.ReportDiagnostic(
                Diagnostic.Create(s_accessRule, operation.Syntax.GetLocation(),
        ...
        foreach (AttributeData attribute in accessedType.GetAttributes())
        {
        ...
        IOperation original)
    {
        switch (operation)
        {
            ...
            case IInvocationOperation invoke:
                {
                    INamedTypeSymbol pureAttribute = context.Compilation.GetTypeByMetadataName(PureAttributeType);

                    foreach (AttributeData attribute in invoke.TargetMethod.GetAttributes())
                    {
    ...
    {
        foreach (INamedTypeSymbol otherType in GetBaseTypesAndThis(type))
        {
